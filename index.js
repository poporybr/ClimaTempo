...
async function successCallback(position) {
    ...
    pSensi.textContent = sens
    pCondic.textContent = condic
  }
function errorCallback(error) {
    switch (error.code) {
      case error.PERMISSION_DENIED:
        alert("Ao negar sua localização, a aplicação pode não funcionar corretamente.");
        break;
...
async function requestAPI(lat, lon){
    try {
        const reponse = await fetch(`https://api.weatherapi.com/v1/current.json?key=54e94c17fdab44f4beb132817231506&q=${lat + ',' + lon}&lang=pt`)
